...
create_descriptor_sets(struct trtl_object *obj, struct swap_chain_data *scd);

static void trtl_object_draw_(struct trtl_object *obj,
			      VkCommandBuffer cmd_buffer,
			      VkPipelineLayout pipeline_layout, int32_t offset)
{
	vkCmdBindDescriptorSets(cmd_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
				pipeline_layout, 0, 1, obj->descriptor_set, 0,
				NULL);
	vkCmdDrawIndexed(cmd_buffer, obj->model->nindices, 1, 0, offset, 0);
}

static uint32_t trtl_object_vertices_get_(struct trtl_object *obj,
...
static uint32_t trtl_object_indices_get_(struct trtl_object *obj,
					 uint32_t **indices)
{
...
static bool trtl_object_update_(struct trtl_object *obj) {
...
struct trtl_object *trtl_object_create(void *ctx, struct swap_chain_data *scd,
				       const char *path, const char *texture)
{
	...
	// FIXME: So leaky
	obj->texture_image_view = create_texture_image_view(
	    scd->render, create_texture_image(scd->render, texture));
...
create_descriptor_sets(struct trtl_object *obj, struct swap_chain_data *scd)
{
	VkDescriptorSet *sets =
	    talloc_zero_array(obj, VkDescriptorSet, obj->nframes);
	...
	if (vkAllocateDescriptorSets(scd->render->device, &alloc_info, sets) !=
	    VK_SUCCESS) {
